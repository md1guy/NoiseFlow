class Noise {
  ...
  double fade(double t) {
    return t * t * t * (t * (t * 6 - 15) + 10);                                     // Fade function, defined by Ken Perlin: 6t^5 - 15t^4 + 10t^3
  }
  
  double basePerlin(double x, double y, double z) {
    ...
    x2 = Lerp(grad(aba, xf, yf - 1, zf),                                            // values we made earlier.
  ...
  double Lerp(double a, double b, double x) {
    return a + x * (b - a);                                                         // For convenience we bind the result to 0 - 1 (theoretical min/max before is [-1, 1])
  }
  ...
  {
    switch(hash & 0xF)                                                              // Pick a random vector from all 12 possible vectors
    {
        case 0x0: return  x + y;
        case 0x1: return -x + y;
        case 0x2: return  x - y;
